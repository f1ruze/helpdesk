        ...
        <div class="d-flex flex-column-fluid">
            <div class="container">
                ...
                <div class="card card-custom example example-compact">
                    @include('backend.includes.form.header', ['page' => 'writings'])
                    <form
                        action="{{ $edit === false ?  route('backend.writings.store') : route('backend.writings.update', ['writing' => $writing->id]) }}"
                        ...
                        <div class="card-body">
                                ...
                                @foreach ($langs as $lang)
                                    <div class="tab-pane fade @if($loop->first) active show @endif"
                                        ...
                                        <div class="d-flex gap-4 justify-content-between flex-wrap">
                                            ...
                                            <span>
                                                @lang('backend.labels.title') 1 ({{ strtoupper($lang->code) }})
                                            </span>
                                                ...
                                                <input type="text" name="title:{{ $lang->code }}"
                                                       ...
                                                       value="{{ isset($writing) ? $writing->translate($lang->code)?->title : old('title:'.$lang->code) }}"
                                            ...
                                            <span>
                                                ...
                                                <input type="text" name="sub_title:{{ $lang->code }}"
                                                       ...
                                                       value="{{ isset($writing) ? $writing->translate($lang->code)?->sub_title : old('sub_title:'.$lang->code) }}"
                                            ...
                                            <span>
                                                ...
                                                <input type="text" name="slug:{{ $lang->code }}"
                                                       id="slug:{{ $lang->code }}"
                                                       value="{{ isset($writing) ? $writing->translate($lang->code)?->slug : old('slug:'.$lang->code) }}"
                                            ...
                                            <div class="d-flex gap-4 justify-content-between flex-wrap">
                                                <div class="form-group d-flex flex-column ">
                                                    ...
                                                    <textarea id="content"
                                                      ...
                                                      {!! isset($writing) ? $writing->translate($lang->code)?->description  : old('description:'.$lang->code) !!}
                                                   </textarea>
                                    ...
                                    <div class="col-lg-12 col-md-9 col-sm-12">
                                        <select class="form-control selectpicker" multiple
                                            ...
                                            @foreach($categories as $category)
                                                <option
                                                    value="{{$category->id}}" {{ isset($writing) && $writing->categories->contains($category->id)  ? 'selected' : ''  }}>{{$category->translate(locale())->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-lg-12 col-md-9 col-sm-12">
                                        <select class="form-control selectpicker" multiple
                                            ...
                                            @foreach($options as $option)
                                                <option
                                                    value="{{$option->id}}" {{ isset($writing) && $writing->options->contains($option->id)   ? 'selected' : ''  }}>{{$option->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <div class="col-lg-12 col-md-9 col-sm-12">
                                            <div class="input-group">
                                                <input id="link" type="date"
                                                       ...
                                                       name="action_date"
                                                       value="{{ isset($writing) ? \Illuminate\Support\Carbon::parse($writing->action_date)->format('Y-m-d') : date('Y-m-d') }}"
                                                       max="{{ date('Y-m-d') }}">
                                    ...
                                    <div class="form-group d-flex flex-column gap-2 pl-3 col-lg-11">
                                        ...
                                        <div class="input-group date d-flex" id="timepicker-container">
                                            <input type="text" class="form-control" name="action_date_time"
                                                   value="{{ isset($writing) ? \Illuminate\Support\Carbon::parse($writing->action_date)->format('h:i:s') : date('h:i:s') }}"
                                    ...
                                    <div class="col-lg-12 col-md-9 col-sm-12">
                                        <div class="input-group">
                                            <input id="order" type="number"
                                                   ...
                                                   name="order"
                                                   value="{{ isset($writing) ? $writing->order : old('order') }}">
                                            @if ($errors->has("order"))
                                ...
                                <div class="form-group d-flex flex-column gap-2 pl-0 col-lg-6">
                                    ...
                                    <div class="col-lg-12 col-md-9 col-sm-12">
                                        <select class="form-control" name="type" id="type">
                                            <option
                                                value="type_1" {{ isset($writing) && $writing->type == 'type_1' ? 'selected' : ''  }}>
                                                type 1
                                            </option>
                                            <option
                                                value="type_2" {{ isset($writing) && $writing->type == 'type_2' ? 'selected' : ''  }}>
                                                type 2
                                            </option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-lg-12 col-md-9 col-sm-12">
                                        <div class="input-group">
                                            <input id="video" type="file"
                                                   ...
                                                   name="video[]"
                                                   value="{{ isset($writing) ? $writing->order : old('video') }}"
                                                   accept="video/*">
                                            @if ($errors->has("video"))
                                    ...
                                    <div class="col-lg-12 col-md-9 col-sm-12">
                                        <div class="input-group">
                                            <input id="video_url" type="text"
                                                   ...
                                                   name="video_url"
                                                   value="{{ isset($writing) ? $writing->video_url : old('video_url') }}">
                            ...
                            <div class="d-flex gap-2 align-items-center w-100">
                                <div class="form-group d-flex flex-column gap-2 pl-0 col-lg-6">
                                    <label for="writer_id" class="col-form-label pl-4">
                                        Writing
                                    </label>
                                    <div class="col-lg-12 col-md-9 col-sm-12">
                                        <select class="form-control" name="writer_id" id="writer_id">
                                            @foreach($writers as $writer)
                                                <option
                                                    value="{{$writer->id}}" {{ isset($writing) && $writing->writer_id == $writer->id   ? 'selected' : ''  }}
                                                >{{$writer->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <span class="switch switch-md switch-icon">
                                            <label>
                                                <input type="checkbox" class="bool" name="confirmed"
                                                       value="{{ isset($writing) ? $writing->confirmed : old('confirmed') }}" {{ (isset($writing) ? old('confirmed',$writing->confirmed) : old('confirmed',1) ) == 1 ? 'checked' : '' }}>
                                                <span></span>
                                            </label>
                                        </span>
                                        </div>
...
{{--                                                       value="{{ isset($writing) ? $writing->for_free : old('for_free') }}" {{ (isset($writing) ? old('for_free',$writing->for_free) : old('for_free',1) ) == 1 ? 'checked' : '' }}>--}}
{{--                                                <span></span>--}}
{{--                                            </label>--}}
{{--                                        </span>--}}
{{--                                        </div>--}}
                                        ...
                                        <span class="switch switch-md switch-icon">
                                            <label>
                                                <input type="checkbox" class="bool" name="status"
                                                       value="{{ isset($writing) ? $writing->status : old('status') }}" {{ (isset($writing) ? old('status',$writing->status) : old('status',1) ) == 1 ? 'checked' : '' }}>
                                                <span></span>
                                            </label>
                                        </span>
                                        </div>
